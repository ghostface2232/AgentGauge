...
- `%APPDATA%\Gauge\api-cost-ledger.json` — present only once the API-cost option has been turned on: per scanned log file, its path, size, how far it was read, and per-day, per-model **token counts**; plus the message/response identifiers already counted. Days older than about two months are pruned. No conversation content, tokens (credentials), or personal information.
