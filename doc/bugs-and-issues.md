...
**실제 수정**: 토글 경로에서 재읽기를 아예 없앴다. `App`이 마지막으로 디스크에 도달한 값을 필드로 들고 있다가 그것을 기준으로 `desired`를 만들고, `TrySave` 실패 시 같은 필드로 되돌린다 — 읽기 실패가 사용자의 선택으로 위장할 여지가 사라진다. 같은 사유가 뷰모드·표시 기준·스파크라인에도 적용되어 셋 다 `TrySave` 결과를 받아 실패 시 스위치를 원래 값으로 되돌리며, 그 아래의 `AppSettingsFile.TrySave`는 읽기 실패를 두 가지로 나눠, 일시적으로 열지 못한 경우(`Unavailable`)에는 write 자체를 거부하고, 파싱 자체가 불가능한 경우(`Unparsable`)에는 원본을 `settings.corrupt-<타임스탬프>.json`으로 옮긴 뒤 새 문서를 쓴다 — 후자까지 거부하면 영구히 거부하게 되어 앱이 어떤 설정도 저장하지 못하는 상태로 남기 때문이다. 그리고 거부된 쓰기는 설정 카드의 경고 문구로 사용자에게 드러난다.
