...
**실제 수정**: 토글 경로에서 재읽기를 아예 없앴다. `App`이 마지막으로 디스크에 도달한 값을 필드로 들고 있다가 그것을 기준으로 `desired`를 만들고, `TrySave` 실패 시 같은 필드로 되돌린다 — 읽기 실패가 사용자의 선택으로 위장할 여지가 사라진다. 같은 사유가 뷰모드·표시 기준·스파크라인에도 적용되어 셋 다 `TrySave` 결과를 받아 실패 시 스위치를 원래 값으로 되돌리며, 그 아래의 `AppSettingsFile.TrySave`는 기존 파일을 읽지 못했을 때 write 자체를 거부한다(빈 기본값으로 덮어써 다른 스토어의 키를 지우는 것이 더 큰 손실이므로).
