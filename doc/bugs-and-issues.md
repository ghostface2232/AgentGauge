...
### L1. 알림 토글 적용 경로가 읽기 실패 시 fail-open — ✅ 수정됨
...
**실제 수정**: 토글 경로에서 재읽기를 아예 없앴다. `App`이 마지막으로 디스크에 도달한 값을 필드로 들고 있다가 그것을 기준으로 `desired`를 만들고, `TrySave` 실패 시 같은 필드로 되돌린다 — 읽기 실패가 사용자의 선택으로 위장할 여지가 사라진다. 같은 사유가 뷰모드·표시 기준·스파크라인에도 적용되어 셋 다 `TrySave` 결과를 받아 실패 시 스위치를 원래 값으로 되돌리며, 그 아래의 `AppSettingsFile.TrySave`는 읽기에 실패하면 이유를 불문하고 write 자체를 거부한다(빈 기본값으로 덮어써 다른 스토어의 키를 지우는 것이 더 큰 손실이므로). 읽어는 봤으나 설정으로 바꿀 수 없는 파일(JSON이 아니거나, JSON이지만 바인딩에 실패하는 경우)에서는 그 거부가 영구화되므로, 원본을 복사해 남기고 메모리에 남은 값으로 채운 새 문서를 시작하는 `TryRecoverCorrupt`을 별도로 둔다. 다만 쓰기 경로가 아니라 설정 패널에서 호출해 사용자가 보고 있는 자리에서만 파일을 교체하고, 열지조차 못한 파일(`Blocked`)은 내용을 알 수 없으므로 건드리지 않는다. 거부된 쓰기와 교체된 파일 모두 설정 카드의 문구로 드러난다.
